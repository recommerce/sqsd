...
class SQSProcessor {
    ...
    constructor(options) {
        ...
        this.processingMesages = this.processingMessages;
        this.shutdown = false;
        ...
        while (true) {
            ...
            try {
                await axios.get(this.options.workerHealthUrl, {
                    timeout: Math.max(1, Math.min(1000, remaining > 0 ? remaining : 1000))
                })
                ...
                if (new Date().getTime() >= deadline)
                    return false;
                await delay(Math.min(1000, deadline - new Date().getTime()))
            }
        }
    }
